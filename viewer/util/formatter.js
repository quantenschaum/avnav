 ...
 * @param maxPlaces = max. number of fractional digits (default = digits-1), negative: fixed value of fractional digits
 * @param leadingZeroes = use leading zeroes instead of spaces
 * returns string with at least digits (+1 if digits<0) (+1 if maxPlaces!=0) characters
 */
const formatFloat=function(number, digits, maxPlaces, leadingZeroes=false) {
    ...
    maxPlaces=clamp(0,maxPlaces,digits-1);
    number=parseFloat(number); // null-->NaN
    if(!isFinite(number)) return '-'.repeat(digits+(signed?1:0)-maxPlaces)+(maxPlaces?'.'+'-'.repeat(maxPlaces):'');
    ...
    decPlaces = clamp(0,decPlaces,maxPlaces);
...
formatFloat.parameters=[
    ...
    {name:'maxPlaces',type:'NUMBER',default:2,list:[0,20],description:"max. number of decimal places (after the decimal point, default = digits-1)"},
 ...
 * @param opt_fixed total number of digits
 * @param opt_fillRight see formatFloat
 */
...
formatDistance.parameters=[
    ...
    {name:'numDigits',type:'NUMBER',default:3,description:"number of digits in total"},
    {name:'fillRight',type:'BOOLEAN',default:1,description:"allow more that 2 fractional digits"},
];
